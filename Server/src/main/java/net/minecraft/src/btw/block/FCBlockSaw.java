	...
	implements FCIBlockMechanical
{
	...
	public FCBlockSaw( int iBlockID )
	{
	...
	public boolean IsCurrentPowerStateValid( World world, int i, int j, int k )
	{
    	boolean bReceivingPower = IsInputtingMechanicalPower( world, i, j, k );
    	boolean bOn = IsBlockOn( world, i, j, k );
	...
	public void ScheduleUpdateIfRequired( World world, int i, int j, int k )
	{
	...
	public void SawBlockToFront( World world, int i, int j, int k, Random random )
	{
		int iFacing = GetFacing( world, i, j, k );
		FCUtilsBlockPos targetPos = new FCUtilsBlockPos( i, j, k, iFacing );
