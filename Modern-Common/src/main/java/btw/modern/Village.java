...
// 1.5.2 vanilla/server Village.java — verbatim port; surface consumed by frozen
// EntityVillager/FCEntityVillager (addOrRenewAgressor, endMatingSeason, func_82683_b,
// getCenter, getVillageRadius, setReputationForPlayer), EntityAIDefendVillage
// (findNearestVillageAggressor, func_82685_c), EntityAIMoveIndoors
// (findNearestDoorUnrestricted), EntityAIMoveThroughVillage (getVillageDoorInfoList),
// EntityAIRestrictOpenDoor (findNearestDoor), EntityAIVillagerMate (getNumVillageDoors,
// getNumVillagers, isMatingSeason), EntityIronGolem (getCenter, getVillageRadius,
...
public class Village
{
    ...
    private boolean isBlockDoor(int par1, int par2, int par3)
    {
        ...
        return var4 <= 0 ? false : ( var4 == Block.doorWood.blockID || var4 == getFcWoodDoorBlockID() );
    	// END FCMOD
    }

    // 1.5.2 (FCMOD) FCBetterThanWolves.fcBlockDoorWood — Village.isBlockDoor and
    // VillageCollection.isWoodenDoorAt also match FC's wood door block; FC code is
    // not visible to Modern-Common at compile time, so the block ID is resolved
    // reflectively and cached (0 while unresolved, which never matches a door ID)
    private static int fcWoodDoorBlockID = 0;
    private static boolean fcWoodDoorResolveFailed = false;

    static int getFcWoodDoorBlockID()
    {
        if (fcWoodDoorBlockID == 0 && !fcWoodDoorResolveFailed)
        {
            try
            {
                Block door = (Block)Class.forName("net.minecraft.src.btw.core.FCBetterThanWolves")
                        .getField("fcBlockDoorWood").get(null);

                if (door != null)
                {
                    fcWoodDoorBlockID = door.blockID;
                }
            }
            catch (Exception e)
            {
                fcWoodDoorResolveFailed = true;
            }
        }

        return fcWoodDoorBlockID;
    }
