...
public class Direction {
    ...
    // 1.5.2 Direction.getMovementDirection (vanilla Direction.java:37) — dominant-axis
    // facing from a motion vector; used by Entity.setInPortal.
    public static int getMovementDirection(double dx, double dz) {
        return MathHelper.abs((float) dx) > MathHelper.abs((float) dz)
                ? (dx > 0.0D ? 1 : 3)
                : (dz > 0.0D ? 2 : 0);
    }
}
