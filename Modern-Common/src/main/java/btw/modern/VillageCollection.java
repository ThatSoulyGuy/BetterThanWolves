...
// 1.5.2 vanilla/server VillageCollection.java — verbatim port; surface consumed by
// frozen EntityVillager/FCEntityVillager (addVillagerPosition, findNearestVillage),
// EntityAIMoveIndoors/EntityAIMoveThroughVillage/EntityAIRestrictOpenDoor/
// EntityAIVillagerMate/EntityIronGolem (findNearestVillage); ticked once per server
// tick from WorldBridge.ModSpecificTick (1.5.2 WorldServer.tick)
public class VillageCollection extends WorldSavedData
{
    ...
    // 1.5.2 (bridge) — the World shim field-initializes villageCollectionObj before
    // any world exists; WorldBridge.ModSpecificTick binds the world via func_82566_a
    // before each tick()
    ...
    {
        // 1.5.2 ((BlockDoor)Block.doorWood).getDoorOrientation — the Block.doorWood shim
        // is a plain ConcreteBlock (not castable to BlockDoor), so the orientation is
        // computed locally from raw metadata (see getDoorOrientation below)
        int var4 = this.getDoorOrientation(par1, par2, par3);
        int var5;
        int var6;
    ...
    // 1.5.2 BlockDoor.getDoorOrientation/getFullMetadata (BlockDoor.java:95/349) —
    // addDoorToNewListIfAppropriate above; inlined because the Block.doorWood shim is
    // not a BlockDoor. The bottom half's metadata (low 2 bits) is the orientation; a
    // block with bit 8 set is the top half, whose bottom half sits one block below
    private int getDoorOrientation(int par1, int par2, int par3)
    {
        int var4 = this.worldObj.getBlockMetadata(par1, par2, par3);

        if ((var4 & 8) != 0)
        {
            return this.worldObj.getBlockMetadata(par1, par2 - 1, par3) & 3;
        }
    ...
    private boolean isWoodenDoorAt(int par1, int par2, int par3)
    {
        ...
        // (var4 > 0 guard added: Village.getFcWoodDoorBlockID() returns 0 while the
        // reflective FC lookup is unresolved, which must never match air)
        return var4 == Block.doorWood.blockID || ( var4 > 0 && var4 == Village.getFcWoodDoorBlockID() );
        // END FCMOD
    }
